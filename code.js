...
var col,
    ...
    undoRedoArrTemp = new Array(line),
    getPresetPosition,     
    start,
    blink;
...
document.getElementById("gridContainer").addEventListener("click", function(e){
    ...
    //saving the array for undo/redo
    //generate a new array on EVERY click to create a new instance of the undoRedoArray (to avoid a reference to the previous undoRedoArr array)
    undoRedoArr = new Array(line);
    for (let i = 0; i < line; i++) { 
       undoRedoArr[i] = new Array(line);
    }
    // copy the screen status to the new array
    for (let i = 0; i < line; i++) {
        for (let j = 0; j < col; j++) {
            if (document.getElementById(i + "-" + j).classList.contains("alive")) {
                undoRedoArr[i][j] = 1;
    ...
    //erasin´ the first grid status of the last 30 movements (if movements are 30+)
    if (undoRedo.grids.length > 30) {
        undoRedo.grids.shift();
        undoRedo.position -= 1;
    }  
    // add the screen status to the undo/redo array and refresh the position to the final of that array
    undoRedo.grids.splice(undoRedo.position + 1, (undoRedo.grids.length - 1) - undoRedo.position, undoRedoArr);
    undoRedo.position = undoRedo.grids.length -1;
    console.log(undoRedo.grids);
